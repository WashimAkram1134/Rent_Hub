  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
      ...
      <div className="flex items-center gap-4 overflow-x-auto pb-4 custom-scrollbar">
        {listings.map((item) => (
          <Link href={`/listings/${(item as any).slug || item.id}`} key={item.id} className="min-w-[140px] group cursor-pointer block">
            <div className="aspect-[4/3] rounded-xl overflow-hidden mb-3 relative bg-slate-100 border border-slate-200 group-hover:border-indigo-300 transition-colors">
              {item.image_url ? (
            ...
            <h4 className="text-[11px] font-bold text-slate-900 truncate group-hover:text-indigo-600 transition-colors">{item.title}</h4>
            {item.status === "PENDING" ? (
